...
async function setupTestData(page: any) {
  ...
  try {
    ...
    const existingProject = await page
...
async function setupTestTranslations(page: any) {
  try {
    ...
    const hasTranslations =
      (await page.locator('[data-testid="translation-item"]').count()) > 0
...
async function setupTestBilling(page: any) {
  try {
    ...
    const needsBilling = await page
      .locator('[data-testid="setup-billing-button"]')
      .isVisible()
