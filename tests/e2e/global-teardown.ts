...
async function globalTeardown(config: FullConfig) {
  ...
  try {
    ...
    const reportFiles = ['e2e-results.json', 'e2e-junit.xml']
    ...
    const summary = {
      ...
      results: {
        ...
        failed: testsFailed,
        successRate:
          testsRun > 0 ? Math.round((testsPassed / testsRun) * 100) : 0,
      },
    ...
    const complianceScore =
      Object.values(vietnameseCompliance).filter(Boolean).length
