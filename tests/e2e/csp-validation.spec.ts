...
test.describe('CSP Violation Prevention', () => {
  ...
  test('Vietnamese theme switching works without CSP violations', async ({
    page,
  }) => {
    await page.goto('/')
  ...
  test('Style tags have proper nonce attributes', async ({ page }) => {
    ...
    const styleTagsWithoutNonce = await page.evaluate(() => {
      ...
      const withoutNonce = Array.from(styleTags).filter(
        tag => !tag.getAttribute('nonce')
      )
  ...
  test('Styled-components integration works', async ({ page }) => {
    ...
    const styledComponentsTags = await page.evaluate(() => {
      ...
      const withoutNonce = Array.from(styledTags).filter(
        tag => !tag.getAttribute('nonce')
      )
      return {
  ...
  test('Accessibility features work without CSP violations', async ({
    page,
  }) => {
    await page.goto('/')
    await page.waitForLoadState('networkidle')
...
test.describe('CSP Policy Validation', () => {
  test('CSP header is correctly set', async ({ page }) => {
    ...
    expect(cspHeader).toContain(
      "'sha256-fmrAi/Sk2PEewIwSMQeP06lkuW9P4P+oXzvxtdiJLss='"
    )
  })
...
test.describe('Performance Impact Assessment', () => {
  test('CSP fixes do not significantly impact performance', async ({
    page,
  }) => {
