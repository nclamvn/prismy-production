...
test.describe('🌍 Vietnamese Translation Core Flows', () => {
  ...
  test('Text translation interface works correctly', async ({ page }) => {
    ...
    const translateLink = page.locator('a:has-text("Dịch"), a:has-text("Translate"), [href*="translate"], [href*="workspace"]').first();
    ...
    const sourceTextArea = page.locator('textarea[placeholder*="Nhập"], textarea[placeholder*="Enter"], [data-testid*="source"]').first();
    const targetArea = page.locator('[data-testid*="target"], [data-testid*="result"], .translation-result').first();
    ...
    if (await sourceTextArea.count() > 0) {
      ...
      const fromLanguage = page.locator('select[name*="from"], [data-testid*="from-lang"], button:has-text("Tiếng Việt")');
      const toLanguage = page.locator('select[name*="to"], [data-testid*="to-lang"], button:has-text("English")');
      
      // Select Vietnamese as source language
      if (await fromLanguage.count() > 0) {
        await fromLanguage.first().click();
        const vietnameseOption = page.locator('option:has-text("Tiếng Việt"), [value="vi"], text=/Vietnamese/i');
      ...
      if (await toLanguage.count() > 0) {
        ...
        const englishOption = page.locator('option:has-text("English"), [value="en"], text=/Tiếng Anh/i');
      ...
      const translateButton = page.locator('button:has-text("Dịch"), button:has-text("Translate"), [data-testid*="translate"]').first();
  ...
  test('Document translation upload works', async ({ page }) => {
    ...
    const docTranslateLink = page.locator('a:has-text("Tài liệu"), a:has-text("Document"), [href*="document"]').first();
    ...
    const fileInput = page.locator('input[type="file"], [data-testid*="upload"]');
    ...
    if (await fileInput.count() > 0) {
      ...
      const uploadArea = page.locator('[data-testid*="drop"], .upload-area, text=/Kéo thả|Drag|Upload/i');
  ...
  test('Translation history and workspace functionality', async ({ page }) => {
    ...
    const historyLink = page.locator('a:has-text("Lịch sử"), a:has-text("History"), [href*="history"]').first();
    ...
    if (await historyLink.count() > 0) {
      ...
      const historyItems = page.locator('[data-testid*="history"], .history-item, .translation-item');
    ...
    const workspaceLink = page.locator('a:has-text("Workspace"), a:has-text("Không gian"), [href*="workspace"]').first();
    ...
    if (await workspaceLink.count() > 0) {
      ...
      const workspaceElements = page.locator('[data-testid*="workspace"], .workspace, .project');
  ...
  test('Translation quality settings are available', async ({ page }) => {
    ...
    const settingsButton = page.locator('button:has-text("Cài đặt"), button:has-text("Settings"), [data-testid*="settings"]').first();
    
    if (await settingsButton.count() > 0) {
      await settingsButton.click();
      
      // Check for quality options
      const qualityOptions = page.locator('text=/Chất lượng|Quality|Độ chính xác|Accuracy/i');
      ...
      const languageSettings = page.locator('text=/Ngôn ngữ|Language|Preference/i');
  ...
  test('Translation API rate limiting is handled gracefully', async ({ page }) => {
    const sourceTextArea = page.locator('textarea[placeholder*="Nhập"], textarea[placeholder*="Enter"], [data-testid*="source"]').first();
    const translateButton = page.locator('button:has-text("Dịch"), button:has-text("Translate"), [data-testid*="translate"]').first();
    ...
    if (await sourceTextArea.count() > 0 && await translateButton.count() > 0) {
      ...
      const rateLimitMessage = page.locator('text=/rate limit|quá nhiều|too many|vượt quá/i, [role="alert"]');
  ...
  test('Character and word count displays correctly', async ({ page }) => {
    ...
    if (await sourceTextArea.count() > 0) {
      const testText = 'Đây là một đoạn văn bản tiếng Việt để kiểm tra tính năng đếm từ và ký tự.';
      await sourceTextArea.fill(testText);
      
      // Look for character/word counters
      const charCounter = page.locator('text=/ký tự|character|chars/i, [data-testid*="char"]');
      const wordCounter = page.locator('text=/từ|word|words/i, [data-testid*="word"]');
  ...
  test('Translation export functionality works', async ({ page }) => {
    ...
    const exportButton = page.locator('button:has-text("Xuất"), button:has-text("Export"), button:has-text("Tải về"), [data-testid*="export"]').first();
    ...
    if (await exportButton.count() > 0) {
      ...
      const vietnameseOptions = page.locator('text=/Định dạng|Format|Xuất file/i');
  ...
  test('Translation accuracy feedback system', async ({ page }) => {
    ...
    const feedbackButtons = page.locator('button[aria-label*="feedback"], button:has-text("👍"), button:has-text("👎"), [data-testid*="feedback"]');
    ...
    if (await feedbackButtons.count() > 0) {
      ...
      const feedbackConfirm = page.locator('text=/Cảm ơn|Thank you|Đã ghi nhận/i, [role="alert"]');
