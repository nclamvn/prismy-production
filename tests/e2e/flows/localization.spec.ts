...
test.describe('🇻🇳 Vietnamese Localization & Cultural Adaptation', () => {
  ...
  test('Vietnamese date and time formatting', async ({ page }) => {
    ...
    const timezoneInfo = page.locator('text=/GMT\+7|ICT|Indochina|Hồ Chí Minh/i');
  ...
  test('Vietnamese text rendering and typography', async ({ page }) => {
    ...
    if (await textInput.count() > 0) {
      const vietnameseTestText = 'Xin chào! Tôi tên là Nguyễn Văn A từ Hồ Chí Minh.';
  ...
  test('Vietnamese address and phone number validation', async ({ page }) => {
    ...
    if (await addressInput.count() > 0) {
      ...
      const vietnameseAddress = '123 Nguyễn Huệ, Phường Bến Nghé, Quận 1, TP.HCM';
  ...
  test('Bilingual content display (Vietnamese-English)', async ({ page }) => {
    ...
    if (await bilingualElements.count() > 0) {
      ...
      const hasVietnamese = /[àáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ]/.test(bilingualText || '');
    ...
    if (await vietnameseSection.count() > 0 && await englishSection.count() > 0) {
