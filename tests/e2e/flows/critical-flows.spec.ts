...
test.describe('🚨 Critical User Flows - Zero Failure Tolerance', () => {
  ...
  test('Homepage loads successfully with Vietnamese elements', async ({ page }) => {
    // Critical: Homepage must load
    await expect(page).toHaveTitle(/Prismy/i);
    
    // Critical: Vietnamese content must be present
    const vietnameseText = page.locator('text=/Dịch thuật|Tiếng Việt|Bản dịch/i');
    ...
    const primaryButton = page.locator('button:has-text("Bắt đầu"), button:has-text("Start"), [data-testid*="start"], [data-testid*="translate"]').first();
  ...
  test('User authentication flow works correctly', async ({ page }) => {
    // Navigate to sign in
    const signInLink = page.locator('a:has-text("Đăng nhập"), a:has-text("Sign"), [href*="signin"], [href*="auth"]').first();
    ...
    if (await signInLink.count() > 0) {
      ...
      const submitButton = page.locator('button[type="submit"], button:has-text("Đăng nhập"), button:has-text("Sign")').first();
  ...
  test('Translation interface is accessible and functional', async ({ page }) => {
    ...
    if (await translateLink.count() > 0) {
      ...
      const targetArea = page.locator('textarea[readonly], [data-testid*="target"], [data-testid*="result"]').first();
      ...
      console.log('⚠️ Translation interface not accessible - may require authentication');
    }
  });
  ...
  test('Navigation is responsive and accessible', async ({ page }) => {
    ...
    if (viewportSize && viewportSize.width < 768) {
      const mobileMenuButton = page.locator('button[aria-label*="menu"], button[aria-expanded], [data-testid*="menu"]').first();
      
      if (await mobileMenuButton.count() > 0) {
        await mobileMenuButton.click();
        
        // Mobile menu should be visible
        const mobileMenu = page.locator('[role="menu"], [data-testid*="mobile-menu"], nav[aria-expanded="true"]');
  ...
  test('Error handling displays user-friendly messages', async ({ page }) => {
    ...
    const homeLink = page.locator('a:has-text("Trang chủ"), a:has-text("Home"), a[href="/"]').first();
  ...
  test('Vietnamese timezone and date formatting', async ({ page }) => {
    ...
    const dateElements = page.locator('[datetime], .date, [data-testid*="date"], [data-testid*="time"]');
    
    if (await dateElements.count() > 0) {
      const dateText = await dateElements.first().textContent();
      
      // Should include Vietnamese month names or proper date format
      const vietnameseDatePattern = /\d{1,2}\/\d{1,2}\/\d{4}|\d{1,2}-\d{1,2}-\d{4}/;
