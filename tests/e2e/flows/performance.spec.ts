...
test.describe('⚡ Performance & Vietnamese Network Optimization', () => {
  ...
  test('Page load performance meets Vietnamese mobile standards', async ({
    page,
  }) => {
  ...
  test('Translation API performance under load', async ({ page }) => {
    ...
    const sourceTextArea = page
      .locator('textarea[placeholder*="Nhập"], [data-testid*="source"]')
      .first()
    const translateButton = page
      .locator('button:has-text("Dịch"), [data-testid*="translate"]')
      .first()
    ...
    if (
      ...
      for (const text of testTexts) {
        ...
        const result = page
          .locator('[data-testid*="target"], .translation-result')
          .first()
  ...
  test('Database query performance simulation', async ({ page }) => {
    ...
    if ((await historyItems.count()) > 0) {
      ...
      await page.waitForSelector('[data-testid*="history"], .history-item', {
        timeout: 5000,
      })
      ...
      const loadMoreButton = page.locator(
        'button:has-text("Tải thêm"), button:has-text("Load more")'
      )
  ...
  test('Network resilience and error recovery', async ({ page }) => {
    ...
    if (
      ...
      const timeoutMessage = page.locator(
        'text=/timeout|hết thời gian|lỗi mạng/i'
      )
  ...
  test('Caching and offline functionality', async ({ page }) => {
    ...
    if (swRegistration) {
      console.log(
        `🔄 Service Worker: ${swRegistration.scope} (active: ${swRegistration.active})`
      )
