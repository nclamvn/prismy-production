...
test.describe('💳 Vietnamese Payment Integration Flows', () => {
  ...
  test('VNPay integration flow works correctly', async ({ page }) => {
    ...
    const pricingLink = page
      .locator(
        'a:has-text("Giá"), a:has-text("Pricing"), a:has-text("Gói"), [href*="pricing"], [href*="plans"]'
      )
      .first()
    ...
    if ((await pricingLink.count()) > 0) {
      ...
      const vnpayOption = page.locator(
        'text=/VNPay|vnpay/i, [data-payment="vnpay"], [value="vnpay"]'
      )
      ...
      if ((await vnpayOption.count()) > 0) {
        ...
        const vnpayElements = page.locator(
          '[data-testid*="vnpay"], .vnpay, text=/VNPay/i'
        )
  ...
  test('MoMo integration flow is available', async ({ page }) => {
    ...
    const paymentButton = page
      .locator(
        'button:has-text("Thanh toán"), button:has-text("Payment"), button:has-text("Nâng cấp"), [data-testid*="payment"]'
      )
      .first()
    ...
    if ((await paymentButton.count()) > 0) {
      ...
      const momoOption = page.locator(
        'text=/MoMo|momo/i, [data-payment="momo"], [value="momo"]'
      )
      ...
      if ((await momoOption.count()) > 0) {
        ...
        const momoElements = page.locator(
          '[data-testid*="momo"], .momo, text=/MoMo/i'
        )
        await expect(momoElements.first()).toBeVisible({ timeout: 5000 })

        // Check for mobile number input (MoMo requirement)
        const phoneInput = page.locator(
          'input[type="tel"], input[placeholder*="số điện thoại"], input[placeholder*="phone"]'
        )
  ...
  test('Stripe integration supports Vietnamese users', async ({ page }) => {
    ...
    const stripeOption = page.locator(
      'text=/Thẻ tín dụng|Credit Card|Stripe/i, [data-payment="stripe"], [data-payment="card"]'
    )
    ...
    if ((await stripeOption.count()) > 0) {
      ...
      const cardInputs = page.locator(
        'input[placeholder*="card"], input[placeholder*="thẻ"], iframe[name*="card"]'
      )
      ...
      if ((await cardInputs.count()) > 0) {
        ...
        const vietnameseLabels = page.locator(
          'label:has-text("Số thẻ"), label:has-text("Tên chủ thẻ"), label:has-text("CVV")'
        )
  ...
  test('Payment security measures are in place', async ({ page }) => {
    ...
    const securityElements = page.locator(
      'text=/Bảo mật|Secure|SSL|An toàn/i, [data-testid*="secure"]'
    )
    ...
    if ((await forms.count()) > 0) {
      const csrfToken = page.locator(
        'input[name*="csrf"], input[name*="_token"], meta[name="csrf-token"]'
      )
  ...
  test('Payment flow handles Vietnamese input validation', async ({ page }) => {
    ...
    const addressInput = page.locator(
      'input[placeholder*="địa chỉ"], input[name*="address"]'
    )
  ...
  test('Payment confirmation displays Vietnamese content', async ({ page }) => {
    ...
    const confirmButton = page.locator(
      'button:has-text("Xác nhận"), button:has-text("Confirm"), [data-testid*="confirm"]'
    )

    if ((await confirmButton.count()) > 0) {
      // Don't actually submit, but check for confirmation elements
      const confirmationElements = page.locator(
        'text=/Xác nhận thanh toán|Confirm payment|Đơn hàng|Order/i'
      )
  ...
  test('Payment pricing tiers are displayed correctly', async ({ page }) => {
    ...
    const pricingTiers = page.locator(
      '[data-testid*="plan"], .pricing-card, .plan'
    )
