...
export function usePerformanceTracking(options: UsePerformanceTrackingOptions = {}): PerformanceTracker {
  const {
    trackPageLoad = true,
  ...
  const recordPageLoadMetrics = useCallback(() => {
    ...
    try {
      ...
      logger.error('Failed to record page load metrics', { error, component: componentName })
    }
  }, [componentName])
  ...
  const recordMetric = useCallback((name: string, value: number, unit: string = 'count') => {
    productionMetrics.recordMetric(
      name,
      value,
      unit,
...
export function useFeatureTracking(componentName: string) {
  ...
  const trackClick = useCallback((feature: string, properties?: Record<string, any>) => {
    trackFeatureUsage(`${feature}_click`, properties)
  }, [trackFeatureUsage])

  const trackView = useCallback((feature: string, properties?: Record<string, any>) => {
    trackFeatureUsage(`${feature}_view`, properties)
  }, [trackFeatureUsage])

  const trackComplete = useCallback((feature: string, properties?: Record<string, any>) => {
...
export function useBusinessMetrics() {
  ...
  const trackDocumentUpload = useCallback((
    ...
    trackBusinessEvent('document_uploaded', fileSize, {
      fileType,
      processingTime,
      sizeCategory: fileSize > 1024 * 1024 ? 'large' : fileSize > 100 * 1024 ? 'medium' : 'small'
    })
  }, [trackBusinessEvent])
