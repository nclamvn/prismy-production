...
async function detectLanguage(text: string): Promise<string> {
  ...
  const englishMatches = englishWords.filter(
    word => sample.includes(' ' + word + ' ') || sample.startsWith(word + ' ')
  ).length
...
async function extractDataFromXLSX(buffer: ArrayBuffer): Promise<any> {
  try {
    ...
    const sheetMatches =
      workbookContent.match(/<sheet[^>]*name="([^"]*)"[^>]*\/>/g) || []
    ...
    if (worksheet1) {
      ...
      const cellMatches =
        worksheetContent.match(/<c[^>]*r="([A-Z]+\d+)"[^>]*>([^<]*)<\/c>/g) ||
        []

      cellData = cellMatches
        .map(match => {
