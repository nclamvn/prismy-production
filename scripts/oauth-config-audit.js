    ...
    try {
      ...
      response.headers.forEach((value, name) => {
        if (
          ...
          name.toLowerCase().includes('cors')
        ) {
...
async function auditProjectUrls() {
  ...
  const currentDeploy =
    'https://prismy-production-1rrddzrb5-nclamvn-gmailcoms-projects.vercel.app'
...
async function auditBrowserCompatibility() {
  ...
  console.log(
    `  Cross-origin: ${!isLocalhost ? 'Yes (requires SameSite=None)' : 'No'}`
  )
