...
class EdgeFunctionTester {
  ...
  async runTest(): Promise<TestReport> {
    ...
    const requestsPerWorker = Math.floor(
      this.config.totalRequests / this.config.concurrentUsers
    )
  ...
  private async createWorker(
    ...
    console.log(
      `👤 Worker ${workerId + 1} starting ${requestCount} requests...`
    )
...
class TestReportGenerator {
  static async generateReport(report: TestReport): Promise<void> {
    ...
    console.log(
      `95th Percentile: ${report.summary.p95ResponseTime.toFixed(2)}ms`
    )
    console.log(
      `99th Percentile: ${report.summary.p99ResponseTime.toFixed(2)}ms`
    )
  ...
  private static analyzePerformance(report: TestReport): void {
    ...
    if (summary.p99ResponseTime > summary.p95ResponseTime * 2) {
      console.log(
        '- High response time variance detected - investigate performance outliers'
      )
    }
  }
