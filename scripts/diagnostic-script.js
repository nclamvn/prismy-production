...
document.addEventListener('securitypolicyviolation', e => {
  ...
  if (violation.sample) {
    ...
    if (
      violation.sample.includes('styled-components') ||
      violation.sample.includes('sc-')
    ) {
    ...
    if (
      violation.directive === 'style-src' &&
      violation.sample.startsWith('{')
    ) {
    ...
    if (
      ...
      violation.sourceFile.includes('react-devtools')
    ) {
      console.warn(
        '🎯 [DIAGNOSTIC] React DevTools CSP violation (development only)'
      )
    }
  }
})
...
try {
  ...
  if (
    window.GoTrueClient ||
    (window.supabase && window.supabase.GoTrueClient)
  ) {
  ...
  console.warn(
    '🔍 [DIAGNOSTIC] Could not intercept GoTrueClient:',
    error.message
  )
}
    ...
    if (!child.hasAttribute('nonce')) {
      const nonce =
        document
          ...
          ?.getAttribute('content') || window.__CSP_NONCE__
...
function detectLoadedLibraries() {
  ...
  const libraryChecks = {
    ...
    'material-ui': () =>
      window.MaterialUI || document.querySelector('[class*="makeStyles"]'),
    react: () => window.React,
    supabase: () => window.supabase || window.__PRISMY_SUPABASE_CLIENT__,
    'next.js': () => window.__NEXT_DATA__,
    'framer-motion': () =>
      window.FramerMotion || document.querySelector('[data-framer-motion]'),
  }
...
window.generateDiagnosticReport = function () {
  const report = {
    ...
    dynamicStyles: {
      ...
      withoutNonce: window.__DYNAMIC_STYLES_LOG__.filter(s => !s.hasNonce)
        .length,
    ...
    environment: {
      ...
      nonce:
        document
          .querySelector('meta[name="csp-nonce"]')
          ?.getAttribute('content') || 'NOT_FOUND',
    },
  }
...
window.printDiagnosticSummary = function () {
  ...
  console.log(
    ...
    report.supabaseInstances.goTrueInstances
  )
  ...
  if (report.cspViolations.total > 0) {
    ...
    Object.entries(report.cspViolations.byDirective).forEach(
      ([directive, count]) => {
