...
async function testSupabaseConnection() {
  try {
    ...
    if (dbError && !dbError.message.includes('relation "user_profiles" does not exist')) {
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 403) {
        warning('Google Translate API key appears to be invalid or not authorized')
      } else {
        warning(`Google Translate API test failed with status: ${response.status}`)
      }
      return false
    }
...
async function testEnvironmentVariables() {
  ...
  for (const varName of requiredVars) {
    ...
    if (!value || value.includes('placeholder') || value.includes('your-')) {
      error(`Required environment variable ${varName} is missing or contains placeholder`)
  ...
  for (const varName of optionalVars) {
    const value = process.env[varName]
    if (!value || value.includes('placeholder') || value.includes('your-')) {
      warning(`Optional environment variable ${varName} is missing or contains placeholder`)
    } else {
      success(`${varName} is configured`)
    }
  }
...
async function runFoundationTests() {
  ...
  if (failed === 0) {
    console.log('')
    success('🎉 All foundation tests passed! Your Prismy installation is ready.')
  } else {
    console.log('')
    warning('⚠️  Some tests failed. Check the issues above and fix them before proceeding.')
  }
