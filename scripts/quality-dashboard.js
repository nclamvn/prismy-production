...
class QualityDashboard {
  ...
  collectCoverageMetrics() {
    ...
    try {
      ...
      execSync('npm test -- --coverage --watchAll=false --silent', { stdio: 'pipe' });
  ...
  collectSecurityMetrics() {
    ...
    try {
      ...
      this.metrics.security = {
        ...
        status: (audit.metadata?.vulnerabilities?.critical || 0) === 0 && 
                (audit.metadata?.vulnerabilities?.high || 0) === 0 ? 'PASS' : 'FAIL'
      };

      // Check ZAP scan results if available
      const zapReportPath = path.join(process.cwd(), 'security', 'baseline-report.json');
      if (fs.existsSync(zapReportPath)) {
        const zapReport = JSON.parse(fs.readFileSync(zapReportPath, 'utf8'));
        
        let zapVulns = { high: 0, medium: 0, low: 0, info: 0 };
  ...
  collectCodeQualityMetrics() {
    ...
    try {
      // ESLint analysis
      const eslintResult = execSync('npx eslint --format=json lib/ components/ app/ || true', {
        stdio: 'pipe',
        encoding: 'utf8'
      });
      ...
      const complexFiles = execSync('find lib/ components/ app/ -name "*.ts" -o -name "*.tsx" | head -20', {
        encoding: 'utf8'
      }).split('\n').filter(Boolean);
  ...
  collectVietnameseMetrics() {
    ...
    try {
      ...
      const paymentFiles = execSync('find lib/ components/ app/ -name "*.ts" -o -name "*.tsx"', {
        encoding: 'utf8'
      }).split('\n').filter(Boolean);
      ...
      const complianceScore = Object.values(metrics.payment_gateways).filter(Boolean).length +
  ...
  calculateCoverageScore() {
    ...
    const lineScore = Math.min(this.metrics.coverage.lines.percentage / 70 * 100, 100);
    const branchScore = Math.min(this.metrics.coverage.branches.percentage / 60 * 100, 100);
  ...
  calculateVietnameseScore() {
    ...
    return Math.round((this.metrics.vietnamese.compliance_score / this.metrics.vietnamese.max_score) * 100);
  }
  ...
  generateRecommendations(scores) {
    ...
    if (scores.coverage < 80) {
      recommendations.push('Increase test coverage to ≥70% lines and ≥60% branches');
    }
    
    if (scores.security < 90) {
      recommendations.push('Address security vulnerabilities and implement security headers');
    }
    
    if (scores.codeQuality < 85) {
      recommendations.push('Fix ESLint errors and TypeScript compilation issues');
    }
    
    if (scores.vietnamese < 80) {
      recommendations.push('Enhance Vietnamese market compliance (payment gateways, localization)');
    }
  ...
  generateMarkdownReport() {
    const { summary, coverage, security, codeQuality, vietnamese } = this.metrics;
...
if (require.main === module) {
  ...
  dashboard.runAnalysis().then(metrics => {
    if (format === 'json') {
      ...
      console.log(`Overall Score: ${metrics.summary.overall_score}/100 (${metrics.summary.grade})`);
