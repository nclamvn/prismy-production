...
export interface DocumentAnalysisRequest {
  ...
  documentType: string
  focus: 'daily_insights' | 'contract_review' | 'financial_analysis' | 'project_status' | 'research_synthesis'
  personality: string
  language?: 'en' | 'vi'
  culturalContext?: 'Vietnam' | 'Thailand' | 'Indonesia' | 'Philippines' | 'Singapore'
}
...
export interface ContractAnalysisRequest {
  documentContent: string
  focus: ('compliance' | 'risks' | 'key_dates' | 'obligations' | 'renewal_terms')[]
...
export class AIProviderManager {
  ...
  private fallbackOrder: ('openai' | 'anthropic' | 'cohere')[] = ['anthropic', 'openai', 'cohere']
  ...
  private async analyzeWithAnthropic(request: DocumentAnalysisRequest): Promise<DocumentAnalysisResult> {
    if (!this.anthropic) {
  ...
  private async analyzeWithOpenAI(request: DocumentAnalysisRequest): Promise<DocumentAnalysisResult> {
    if (!this.openai) {
  ...
  private async analyzeWithCohere(request: DocumentAnalysisRequest): Promise<DocumentAnalysisResult> {
  ...
  private buildAnalysisPrompt(request: DocumentAnalysisRequest): string {
    const culturalContext = request.culturalContext === 'Vietnam' ? 
...
`
  ...
  private getPersonalityContext(personality: string): string {
    const personalities = {
      'legal': 'You are a meticulous legal advisor focused on compliance, risk assessment, and regulatory requirements.',
      'financial': 'You are an analytical financial expert focused on budget analysis, cost optimization, and financial performance.',
      'project': 'You are an organized project manager focused on timelines, deliverables, and resource coordination.',
      'research': 'You are a thorough research specialist focused on knowledge synthesis, insights discovery, and data analysis.',
      'general': 'You are a versatile business analyst focused on general productivity and document organization.'
    }
    
    return personalities[personality as keyof typeof personalities] || personalities.general
  }

  private getFocusInstructions(focus: DocumentAnalysisRequest['focus']): string {
    const focuses = {
      'daily_insights': 'Provide daily actionable insights, progress updates, and immediate next steps.',
      'contract_review': 'Analyze contract terms, identify risks, extract key dates, and assess compliance requirements.',
      'financial_analysis': 'Examine financial data, budget performance, cost trends, and investment recommendations.',
      'project_status': 'Evaluate project progress, timeline adherence, resource utilization, and milestone tracking.',
      'research_synthesis': 'Synthesize research findings, identify patterns, extract key insights, and recommend follow-up research.'
    }
    
    return focuses[focus]
  }

  private parseAnalysisResponse(content: string, metadata: any): DocumentAnalysisResult {
    try {
      ...
      return {
        insights: content,
        confidence: 0.75,
        keyPoints: this.extractBulletPoints(content, /Key Points?:|Main Points?:/i),
        actionItems: this.extractBulletPoints(content, /Action Items?:|Next Steps?:/i),
        risks: this.extractBulletPoints(content, /Risks?:|Concerns?:/i),
        deadlines: this.extractDates(content),
        recommendations: this.extractBulletPoints(content, /Recommendations?:|Suggestions?:/i),
        metadata
      }
    } catch (error) {
  ...
  private extractDates(text: string): Date[] {
    const datePattern = /\b(\d{4}-\d{2}-\d{2}|\d{2}\/\d{2}\/\d{4}|\d{1,2}\s+(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{4})/gi
  ...
  async analyzeContract(request: ContractAnalysisRequest): Promise<ContractAnalysisResult> {
    const analysisRequest: DocumentAnalysisRequest = {
      ...
      culturalContext: request.jurisdiction === 'Vietnam' ? 'Vietnam' : undefined
    }
    ...
    return {
      ...
      complianceIssues: this.parseComplianceIssues(result.recommendations || []),
  ...
  private parseContractObligations(actionItems: string[]): ContractObligation[] {
  ...
  private parseComplianceIssues(recommendations: string[]): ComplianceIssue[] {
    return recommendations.filter(rec => rec.toLowerCase().includes('compliance')).map(rec => ({
  ...
  private classifyRiskType(risk: string): ContractRisk['type'] {
    ...
    if (lower.includes('financial') || lower.includes('cost') || lower.includes('payment')) return 'financial'
    if (lower.includes('operational') || lower.includes('delivery') || lower.includes('performance')) return 'operational'
   ...
   */
  async detectProjects(request: ProjectDetectionRequest): Promise<ProjectDetectionResult> {
    // Simplified implementation for now
    const projectKeywords = ['project', 'initiative', 'campaign', 'program', 'development']
    ...
    return {
      ...
      confidence: 0.7,
      projectInsights: Array.from(potentialProjects).slice(0, 3).map(name => ({
        projectName: name,
        status: 'active' as const,
        progress: Math.floor(Math.random() * 100),
        keyDocuments: request.documents.filter(doc => 
   ...
   */
  async assessAgentCompatibility(request: AgentCompatibilityRequest): Promise<AgentCompatibilityResult> {
    ...
    if ((agent1.specialty.includes('legal') && agent2.specialty.includes('financial')) ||
      ...
      synergyScore = 0.9
    } else if ((agent1.specialty.includes('project') && agent2.specialty.includes('research')) ||
