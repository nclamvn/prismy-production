...
export class ModelRouter {
  ...
  async getOptimalModel(
    ...
    try {
      ...
      const validModels = scoredModels.filter(
        model =>
      ...
      const rankedModels = validModels.sort(
      ...
      const reasonCode = this.generateReasonCode(
        bestModel,
        request,
        userSpending
      )
      ...
      for (const [provider, stats] of providerStatsMap) {
        if (stats.totalRequests > 0) {
          this.providerStats.set(provider, {
            ...
            costEfficiency:
              stats.totalQuality / Math.max(stats.totalCost, 0.001),
  ...
  private async getUserSpending(userId: string): Promise<number> {
    try {
      ...
      return (
        data?.reduce((total, log) => total + (log.metadata?.cost || 0), 0) || 0
      )
    } catch (error) {
  ...
  private isModelAvailableForTier(
    provider: string,
    model: string,
    tier: UserTier
  ): boolean {
  ...
  private getQualityScore(model: any, request: ModelRoutingRequest): number {
    ...
    if (request.complexity === TaskComplexity.EXPERT) {
      baseQuality =
        model.model.includes('opus') || model.model.includes('gpt-4')
          ? baseQuality
          : baseQuality * 0.8
    }
    ...
    if (request.languagePair) {
      ...
      if (
        ['zh', 'ja', 'ar', 'hi'].includes(source) ||
        ['zh', 'ja', 'ar', 'hi'].includes(target)
      ) {
  ...
  private getFallbackRecommendation(
    ...
    const fallbackModel =
      request.userTier === UserTier.FREE
