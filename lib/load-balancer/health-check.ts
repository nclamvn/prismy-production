 ...
 */
async function performHealthChecks(): Promise<{ checks: HealthCheckResult['checks'] }> {
 ...
 */
async function performDatabaseCheck(): Promise<HealthCheckResult['checks'][string]> {
  const startTime = Date.now();
 ...
 * Redis health check
 */
async function performRedisCheck(): Promise<HealthCheckResult['checks'][string]> {
  const startTime = Date.now();
 ...
 */
async function performExternalApiCheck(): Promise<HealthCheckResult['checks'][string]> {
  const startTime = Date.now();
 ...
 */
async function performStorageCheck(): Promise<HealthCheckResult['checks'][string]> {
  const startTime = Date.now();
 ...
 */
async function performTranslationServiceCheck(): Promise<HealthCheckResult['checks'][string]> {
  const startTime = Date.now();
 ...
 * Memory health check
 */
async function performMemoryCheck(): Promise<HealthCheckResult['checks'][string]> {
  const startTime = Date.now();
 ...
 * Disk health check
 */
async function performDiskCheck(): Promise<HealthCheckResult['checks'][string]> {
  const startTime = Date.now();
...
function getHttpStatusForHealth(status: 'healthy' | 'degraded' | 'unhealthy'): number {
  switch (status) {
    case 'healthy':
      return 200;
    case 'degraded':
