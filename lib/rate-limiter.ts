...
export async function getRateLimitForTier(
  ...
  if (useRedis && rateLimiters.translation[tier]) {
    try {
      ...
      console.error(`Redis rate limiting failed for tier ${tier}, falling back to in-memory:`, error)
      return fallbackRateLimit(clientId, limit, windowMs)
    }
  }
...
export async function checkAuthRateLimit(identifier: string): Promise<RateLimitResult> {
  if (useRedis && rateLimiters.auth) {
    try {
      ...
      console.error('Redis auth rate limiting failed, falling back to in-memory:', error)
...
export async function checkPaymentRateLimit(identifier: string): Promise<RateLimitResult> {
  if (useRedis && rateLimiters.payment) {
    try {
      ...
      console.error('Redis payment rate limiting failed, falling back to in-memory:', error)
