...
const emailString = z.string().email().transform((val) => validator.normalizeEmail(val) || val)

// Translation request validation
export const translationSchema = z.object({
  text: z.string()
    .min(1, 'Text cannot be empty')
    .max(10000, 'Text too long (maximum 10,000 characters)')
  ...
  sourceLanguage: z.string()
    ...
    .refine(
      (lang) => ['en', 'vi', 'fr', 'es', 'de', 'it', 'pt', 'ru', 'ja', 'ko', 'zh'].includes(lang),
...
export const vnpayCallbackSchema = z.object({
  ...
  vnp_BankTranNo: z.string().max(255, 'Bank transaction number too long').optional(),
  vnp_CardType: z.string().max(20, 'Card type too long').optional(),
  vnp_OrderInfo: z.string().max(255, 'Order info too long').transform((val) => sanitizeHtml(val.trim())),
  ...
  vnp_TransactionStatus: z.string().regex(/^\d{2}$/, 'Invalid transaction status format'),
...
export const momoCallbackSchema = z.object({
  ...
  orderInfo: z.string().max(255, 'Order info too long').transform((val) => sanitizeHtml(val.trim())),
  ...
  message: z.string().max(255, 'Message too long').transform((val) => sanitizeHtml(val.trim())),
...
export const fileUploadSchema = z.object({
  ...
  fileType: z.enum(['text/plain', 'application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'], {
