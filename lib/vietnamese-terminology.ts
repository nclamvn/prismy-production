...
export function getTerminology(
  englishTerm: string,
  formalityLevel: 'formal' | 'informal' | 'business'
): string {
  const entry = VIETNAMESE_TERMINOLOGY.find(
...
export function getAddressing(
  ...
  const addressing = VIETNAMESE_ADDRESSING.find(
    a => a.context === context && a.relationship === relationship
  )
...
export function getDialectVariation(
  ...
  const variation =
    DIALECT_VARIATIONS[dialect].examples[
      term as keyof typeof DIALECT_VARIATIONS.northern.examples
    ]
...
export function getCulturalContext(
  sector: string,
  aspect: 'terminology' | 'formality' | 'communication_style'
): string {
  const context =
    CULTURAL_CONTEXTS.business_sectors[
      sector as keyof typeof CULTURAL_CONTEXTS.business_sectors
    ]
