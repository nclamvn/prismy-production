...
export class PerformanceMonitor {
   ...
   * Record memory usage
   */
  recordMemoryUsage(componentName: string, tags: Record<string, string> = {}): void {
    try {
  ...
  async measureDatabaseQuery<T>(
    ...
    try {
      ...
      const duration = this.endTimer(operationId, 'database_query_time', {
      ...
      this.endTimer(operationId, 'database_query_time', {
        query_name: queryName,
        status: 'error'
  ...
  async getPerformanceReport(
    ...
    try {
      ...
      const responseTimeMetrics = dbMetrics?.filter(m => m.metric_name === 'api_response_time') || []
      const responseTimes = responseTimeMetrics.map(m => m.metric_value).sort((a, b) => a - b)
      ...
      const errorMetrics = dbMetrics?.filter(m => 
      ...
      logger.error('Failed to generate performance report', { error, timeRange })
      throw error
    }
  }
  ...
  private identifySlowQueries(queryMetrics: any[]): Array<{
    ...
    const queryStats = new Map<string, { totalDuration: number; count: number; maxDuration: number }>()
    ...
    queryMetrics.forEach(metric => {
      ...
      if (!queryStats.has(queryName)) {
        queryStats.set(queryName, { totalDuration: 0, count: 0, maxDuration: 0 })
      }
  ...
  private identifyBottlenecks(metrics: any[]): Array<{
    ...
    if (avgMemoryUsage > 512) { // > 512MB
      bottlenecks.push({
        ...
        recommendation: 'Review memory leaks, optimize data structures, or increase memory allocation'
      })
    }

    // Check for database performance
    const dbMetrics = metrics.filter(m => m.metric_name === 'database_query_time')
    const avgDbTime = dbMetrics.length > 0 
      ? dbMetrics.reduce((sum, m) => sum + m.metric_value, 0) / dbMetrics.length 
      : 0
