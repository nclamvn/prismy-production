...
class UIAuditor {
  ...
  private async analyzeFile(filePath: string): Promise<void> {
    try {
      const content = await fs.readFile(path.join(this.rootDir, filePath), 'utf8');
   ...
   */
  private extractComponentInfo(filePath: string, content: string): ComponentInventory {
  ...
  private extractTailwindClasses(content: string, filePath: string): void {
    ...
    for (const cls of classes) {
      const existing = this.report.tailwindClasses.find(item => item.class === cls);
  ...
  private categorizeTailwindClass(cls: string): string {
    ...
    if (baseClass.match(/^(p|px|py|pt|pb|pl|pr|m|mx|my|mt|mb|ml|mr|space|gap)-/)) return 'spacing';
   ...
   */
  private extractPropsInterface(content: string, componentName: string): string | undefined {
  ...
  private checkVietnameseContent(content: string): void {
    ...
    if (content.includes('locale') || content.includes('vi-VN') || content.includes('en-US')) {
  ...
  private analyzeDuplicatePatterns(): void {
    ...
    for (const [category, classes] of classGroups) {
      ...
      for (const [cls, count] of classCounts) {
        if (count > 3) { // Used in more than 3 places
          this.report.duplicatePatterns.push({
            pattern: cls,
            count,
            files: this.report.tailwindClasses.find(item => item.class === cls)?.files || [],
  ...
  private analyzeVietnameseSpecific(): void {
    ...
    if (this.report.vietnameseSpecific.vndCurrencyUsage > 0) {
      this.report.recommendations.push('🇻🇳 Create VND currency formatting utilities');
    }

    if (this.report.vietnameseSpecific.vietnameseTextDetected > 0) {
      this.report.recommendations.push('🇻🇳 Ensure Vietnamese diacritics are properly handled in fonts');
    }
  }
  ...
  private generateRecommendations(): void {
    ...
    const colorClasses = tailwindClasses.filter(item => item.category === 'colors' && item.count > 2);
    if (colorClasses.length > 10) {
      this.report.recommendations.push('🎨 Create a unified color palette - detected multiple color variations');
    }

    // Typography recommendations
    const typographyClasses = tailwindClasses.filter(item => item.category === 'typography' && item.count > 2);
    if (typographyClasses.length > 8) {
      this.report.recommendations.push('📝 Standardize typography scale - detected inconsistent text sizes');
    }

    // Spacing recommendations
    const spacingClasses = tailwindClasses.filter(item => item.category === 'spacing' && item.count > 2);
    if (spacingClasses.length > 15) {
      this.report.recommendations.push('📏 Create spacing tokens - detected inconsistent spacing patterns');
    }

    // Component recommendations
    const buttonLikeComponents = components.filter(c => 
      c.name.toLowerCase().includes('button') || 
      c.tailwindClasses.some(cls => cls.includes('bg-') && cls.includes('text-'))
    );
    if (buttonLikeComponents.length > 3) {
      this.report.recommendations.push('🔘 Consolidate button variants into a single Button component');
    }

    // Duplicate pattern recommendations
    if (duplicatePatterns.length > 5) {
      this.report.recommendations.push('🔄 High number of duplicate patterns detected - prioritize component extraction');
    }
  }
  ...
  private calculateSummary(): void {
    this.report.summary = {
      ...
      totalComponents: this.report.components.filter(c => c.type === 'component').length,
      uniqueTailwindClasses: this.report.tailwindClasses.length,
      duplicatePatterns: this.report.duplicatePatterns.length,
      inlineStyles: this.report.components.reduce((sum, c) => sum + c.inlineStyles.length, 0)
    };
  }
...
async function main() {
  ...
  try {
    ...
    await auditor.exportToJSON(path.join(outputDir, `ui-audit-${timestamp}.json`));
    await auditor.exportToCSV(path.join(outputDir, `ui-inventory-${timestamp}.csv`));

    // Print summary
    console.log('\n📊 UI Audit Summary:');
    console.log(`   📁 Files analyzed: ${report.summary.totalFiles}`);
    console.log(`   🧩 Components found: ${report.summary.totalComponents}`);
    console.log(`   🎨 Unique Tailwind classes: ${report.summary.uniqueTailwindClasses}`);
    console.log(`   🔄 Duplicate patterns: ${report.summary.duplicatePatterns}`);
    console.log(`   💅 Inline styles: ${report.summary.inlineStyles}`);
    console.log(`\n🇻🇳 Vietnamese-specific:`);
    console.log(`   ₫ VND currency usage: ${report.vietnameseSpecific.vndCurrencyUsage}`);
    console.log(`   📝 Vietnamese text detected: ${report.vietnameseSpecific.vietnameseTextDetected}`);
