...
async function getCurrentServiceState() {
  ...
  if (!service) {
    throw new Error(
      `Service ${SERVICE_NAME} not found in cluster ${CLUSTER_NAME}`
    )
  }
...
async function getScalingMetrics() {
  ...
  metrics.memory = await getMetric(
    'AWS/ECS',
    'MemoryUtilization',
    startTime,
    endTime,
  ...
  metrics.requests = await getMetric(
    'AWS/ApplicationELB',
  ...
  metrics.queueDepth = await getMetric(
    'Prismy/Application',
    'QueueDepth',
    startTime,
    endTime,
  ...
  metrics.activeUsers = await getMetric(
    'Prismy/Application',
    'ActiveUsers',
    startTime,
    endTime
...
async function getMetric(
  ...
  try {
    ...
    const latest = result.Datapoints.sort(
      (a, b) => b.Timestamp - a.Timestamp
    )[0]
    const average =
      result.Datapoints.reduce((sum, dp) => sum + dp.Average, 0) /
    ...
    console.warn(
      `Failed to get metric ${namespace}/${metricName}:`,
      error.message
    )
    return { average: 0, maximum: 0, datapoints: 0 }
  }
...
async function makeScalingDecision(serviceState, metrics) {
  ...
  const minCapacity = Math.max(
    MIN_CAPACITY,
    businessHoursConstraint.minCapacity
  )
...
function evaluateScalingSignals(metrics, currentCapacity) {
  ...
  if (metrics.cpu.datapoints > 0) {
    if (metrics.cpu.average > SCALING_CONFIG.cpu.scaleUpThreshold) {
      signals.cpu =
        (metrics.cpu.average - SCALING_CONFIG.cpu.scaleUpThreshold) / 30 // Normalize to 0-1
    } else if (metrics.cpu.average < SCALING_CONFIG.cpu.scaleDownThreshold) {
      signals.cpu =
        (metrics.cpu.average - SCALING_CONFIG.cpu.scaleDownThreshold) / 30 // Negative value
    }
  }
  ...
  if (metrics.memory.datapoints > 0) {
    if (metrics.memory.average > SCALING_CONFIG.memory.scaleUpThreshold) {
      signals.memory =
        (metrics.memory.average - SCALING_CONFIG.memory.scaleUpThreshold) / 20
    } else if (
      metrics.memory.average < SCALING_CONFIG.memory.scaleDownThreshold
    ) {
      signals.memory =
        (metrics.memory.average - SCALING_CONFIG.memory.scaleDownThreshold) / 40
    }
  }
  ...
  if (metrics.requests.datapoints > 0) {
    ...
    if (requestsPerTask > SCALING_CONFIG.requests.scaleUpThreshold) {
      signals.requests =
        (requestsPerTask - SCALING_CONFIG.requests.scaleUpThreshold) / 500
    } else if (requestsPerTask < SCALING_CONFIG.requests.scaleDownThreshold) {
      signals.requests =
        (requestsPerTask - SCALING_CONFIG.requests.scaleDownThreshold) / 200
    }
  }
  ...
  if (metrics.queueDepth.datapoints > 0) {
    if (metrics.queueDepth.average > SCALING_CONFIG.queue.scaleUpThreshold) {
      signals.queue =
        (metrics.queueDepth.average - SCALING_CONFIG.queue.scaleUpThreshold) /
        100
    } else if (
      metrics.queueDepth.average < SCALING_CONFIG.queue.scaleDownThreshold
    ) {
      signals.queue =
        (metrics.queueDepth.average - SCALING_CONFIG.queue.scaleDownThreshold) /
        10
  ...
  if (metrics.activeUsers.datapoints > 0) {
    ...
    if (usersPerTask > 50) {
      // Scale up if more than 50 users per task
      signals.activeUsers = (usersPerTask - 50) / 50
    } else if (usersPerTask < 10) {
      // Scale down if less than 10 users per task
...
function getBusinessHoursConstraint() {
  ...
  const isBusinessHours =
    hour >= SCALING_CONFIG.businessHours.weekdays.start &&
          ...
          StepAdjustments: [
            {
              ...
              ScalingAdjustment:
                decision.targetCapacity - decision.currentCapacity,
            },
          ],
        },
      })
