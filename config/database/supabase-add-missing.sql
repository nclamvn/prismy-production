-- Add only missing components to existing schema
-- This script checks what exists and only adds what's missing

-- Only create credits table if it doesn't exist
DO $$
BEGIN
...
-- Only create tasks table if it doesn't exist
DO $$
BEGIN
...
-- Add missing columns to invitations table if needed
DO $$
BEGIN
...
-- Create or replace the user credit balance view
...
-- Drop and recreate functions to ensure they're correct
...
DROP FUNCTION IF EXISTS create_invitation_code(UUID, TEXT, INTEGER, TIMESTAMP WITH TIME ZONE, INTEGER);
...
-- Create a welcome invitation code for testing (only if it doesn't exist)
...
SELECT 'Setup completed! Available invitation codes:' as message;
SELECT code, max_uses, current_uses, credit_amount, is_active 
FROM invitations 
