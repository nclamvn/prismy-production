...
export function PipelineProvider({ children }: { children: React.ReactNode }) {
  ...
  const orchestratorRef = useRef<PipelineOrchestrator>(
  ...
  const processRequest = useCallback(
    async (
      ...
      try {
        const response =
          await orchestratorRef.current.processRequest(fullRequest)
  ...
  const translation = {
        ...
        return processRequest({
          ...
          input: {
            ...
            metadata: {
              ...
              targetLang:
                options.targetLang || (language === 'vi' ? 'en' : 'vi'),
            },
            preferences: {
              language,
    ...
    detectLanguage: useCallback(
      async (text: string): Promise<PipelineResponse> => {
        return processRequest({
          mode: 'translation',
          type: 'query',
          input: {
  ...
  const documents = {
    ...
    extractText: useCallback(
      async (file: File): Promise<PipelineResponse> => {
        return processRequest({
          mode: 'documents',
          type: 'text',
          input: {
  ...
  const intelligence = {
    ...
    analyzeContent: useCallback(
      async (content: string, type: string): Promise<PipelineResponse> => {
        return processRequest({
          mode: 'intelligence',
          type: 'text',
          input: {
  ...
  const analytics = {
    getUserMetrics: useCallback(
      async (timeRange: string = '30d'): Promise<PipelineResponse> => {
        return processRequest({
          mode: 'analytics',
          type: 'query',
          input: {
...
export function usePipelineMetrics() {
  ...
  return {
    ...
    successRate:
      responses.size > 0
        ? (Array.from(responses.values()).filter(r => r.status === 'completed')
            .length /
            responses.size) *
          100
        : 0,
